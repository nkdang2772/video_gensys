...
## Video GenSystem bridge (v1.1)

Extension có thể nhận trực tiếp `image_gen` job từ Video GenSystem qua localhost:

1. Đặt một token dài ít nhất 16 ký tự trong shell chạy worker:
   ```powershell
   $env:VIDEO_GENSYSTEM_FLOW_BRIDGE_TOKEN = "thay-bang-token-rieng-cua-ban"
   ```
2. Reload unpacked extension sau khi cập nhật source.
3. Mở một project Google Flow và mở side panel `h2dev_flow`.
4. Trong mục **Video GenSystem bridge**, nhập URL `http://127.0.0.1:8765`, nhập cùng token rồi bật switch.
5. Giữ side panel mở, đóng DevTools trên tab Flow và chạy image worker.

Bridge chỉ bind localhost, yêu cầu token cho mọi request và chỉ chấp nhận download path dạng `video_gensystem_bridge/<task-id>.png`. Token không được lưu trong database Video GenSystem. Reference image đã pin được chuyển thẳng từ worker tới file input của Flow; nếu Google đổi UI upload, cần cập nhật selector trong `content.js`.
