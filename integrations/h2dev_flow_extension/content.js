...
async function uploadReferenceImages(references) {
  if (!Array.isArray(references) || references.length === 0) return { ok: true };
  if (references.length > 8) return { ok: false, error: "Tối đa 8 reference cho một task." };
  let input = document.querySelector('input[type="file"][accept*="image"]');
  if (!input) {
    const uploadButton = [...document.querySelectorAll('button, [role="button"]')].find((button) =>
      /upload|reference|image|ảnh|tải lên|add media/i.test(
  ...
  if (!input) return { ok: false, error: "Không tìm thấy input upload reference trên Flow." };
  if (!input.multiple && references.length > 1) {
    return { ok: false, error: "Flow hiện chỉ cho upload một reference ở input đã tìm thấy." };
  }
  try {
    const transfer = new DataTransfer();
    references.forEach((reference, index) => {
      if (!reference || typeof reference.data_url !== "string" || !reference.data_url.startsWith("data:image/")) {
        throw new Error(`Reference ${index + 1} không hợp lệ.`);
      }
      const [header, encoded] = reference.data_url.split(",", 2);
      const mime = header.match(/^data:([^;]+);base64$/)?.[1];
      if (!mime || !encoded) throw new Error(`Reference ${index + 1} không phải base64 image.`);
      const binary = atob(encoded);
      const bytes = new Uint8Array(binary.length);
      for (let i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i);
      const safeName = String(reference.name || `reference_${index + 1}.png`).replace(/[\\/:*?"<>|\r\n]/g, "_");
      transfer.items.add(new File([bytes], safeName, { type: mime }));
    });
...
if (!window.__H2DEV_FLOW_LISTENER__) {
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    if (msg.type === "UPLOAD_REFERENCES") {
      uploadReferenceImages(msg.references)
