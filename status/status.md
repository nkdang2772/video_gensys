...
**Gate chính thức:** **Bước 1–16 — PASS; Bước 17 — đang revalidate**
**Trạng thái:** Code lịch sử tồn tại đến Bước 30, nhưng chỉ Bước 1–16 đã đủ branch/PR/CI/merge để được đánh PASS chính thức. Bước 17 đã đạt test local nhưng chưa được đánh PASS trước khi có PR, CI xanh và merge; Bước 18 chưa được bắt đầu.
...
- **Bước 17 đang revalidate** trên branch `codex/step17-revalidation`: chỉnh inline `visual_description` 10 shot qua cùng persistence path của UI; filter Scene 1 đúng 10 shot; bulk assign character + primary cho 20 shot; chosen audio preview được render. Không chọn shot khi bulk hiển thị lỗi (BUG-040). Targeted **13/13 pass**, full regression **120/120 pass**; PR/CI/merge còn chờ.
- Các mô tả “đã hoàn thành” bên dưới là inventory implementation lịch sử, không phải dấu check DoD cho Bước 17–30.
