...
**Tính năng Visual-first — local acceptance PASS, live Flow chờ token bridge:** branch `codex/visual-first-reference-flow`. Migration `0003` bổ sung prompt/alias cho Reference và `planned_duration_sec` cho Shot nhưng vẫn giữ đúng 10 bảng. CLI `visual setup` idempotent đã nhập bộ production hiện tại thành 24 character prompt + 10 location prompt + 233 shot, toàn bộ shot có timeline tạm 4 giây; mapping tổng quát gán character cho 231/233 và location cho 25/233, các shot chưa chắc chắn được giữ để review. Reference Library sinh trực tiếp Google Flow thành immutable ReferenceVersion, có nút sync pin cho Episode hiện hữu. Preview/Motion/QA/OTIO export có visual-first mode; voice nhập sau tự thay thời lượng tạm. Database thật đạt revision `0003`, WAL, busy timeout 5000 ms, `alembic check` sạch; full regression **145/145 pass**. Chưa đánh live Flow PASS vì process hiện chưa có bridge token giống extension.
