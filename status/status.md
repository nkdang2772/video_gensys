...
**Tính năng sau Bước 30 — Auto timeline OTIO/OTIOZ PASS:** branch `codex/otio-auto-timeline`, PR [#35](https://github.com/nkdang2772/video_gensys/pull/35), 2/2 GitHub checks xanh và merge commit `d09cc1c` trên `main`. Export package sinh đồng thời `timeline.otio` và `timeline.otioz`, tự xếp visual ưu tiên motion rồi image trên V1, voice/padding trên A1 và cross-dissolve đơn giản có thể chỉnh từ UI (0 giây = hard cut). Live acceptance bằng DaVinci Resolve Free 21.0.4 đã import gói OTIOZ 3 shot thành timeline 3 giây, đủ 3 visual + 3 voice + 2 transition; scrub hiển thị đúng cả ba visual và Edit page cho phép chỉnh clip/transition. Focused **6/6 pass**, full regression local/CI **138/138 pass**, `pip check` sạch, Alembic upgrade/check sạch trên database acceptance.

**Tính năng Visual-first — PASS:** branch `codex/visual-first-reference-flow`, PR [#37](https://github.com/nkdang2772/video_gensys/pull/37), **2/2 checks** và merge commit `de513ed` trên `main`. Migration `0003` bổ sung prompt/alias cho Reference và `planned_duration_sec` cho Shot nhưng vẫn giữ đúng 10 bảng. CLI `visual setup` idempotent đã nhập bộ production hiện tại thành 24 character prompt + 10 location prompt + 233 shot, toàn bộ shot có timeline tạm 4 giây; mapping tổng quát gán character cho 231/233 và location cho 25/233, các shot chưa chắc chắn được giữ để review. Reference Library sinh trực tiếp Google Flow thành immutable ReferenceVersion, có nút sync pin cho Episode hiện hữu. Preview/Motion/QA/OTIO export có visual-first mode; voice nhập sau tự thay thời lượng tạm. Database thật đạt revision `0003`, WAL, busy timeout 5000 ms, `alembic check` sạch; full regression **145/145 pass**. Live Google Flow đã sinh `nay_v001.png` 1376×768, 551.975 byte, SHA-256 `c6a25db6346c15fa0b627d0b4e4f1ab0b011adaa17ac7f0e1a4ec59328f8ae53`, lưu thành ReferenceVersion v1 với descriptor provider/prompt đầy đủ và sync thêm 1 EpisodeReferencePin.
...
- **Bước 12 PASS:** branch `codex/step12-revalidation`, PR [#12](https://github.com/nkdang2772/video_gensys/pull/12), 2/2 GitHub checks xanh và merge commit `ca05527` trên `main`.
...
- **Bước 15 PASS:** branch `codex/step15-revalidation`, PR [#15](https://github.com/nkdang2772/video_gensys/pull/15), 2/2 GitHub checks xanh và merge commit `641da86` trên `main`.
- Bằng chứng Bước 15: Streamlit AppTest list/create/open Series và Episode bằng fixture “Tam Quốc”/“Xích Bích”; tên Series rỗng hiển thị lỗi và không tạo record; targeted **3/3 pass**, full CI **115/115 pass**.
- **Bước 16 PASS:** branch `codex/step16-revalidation`, PR [#17](https://github.com/nkdang2772/video_gensys/pull/17), 2/2 GitHub checks xanh và merge commit `be388e8` trên `main`.
- Bằng chứng Bước 16: AppTest import script và **80/80 WAV** qua UI, hiển thị shot list 80 dòng; thiếu script hiển thị lỗi và không tạo Shot. BUG-039 đã đóng bằng retry giới hạn cho FFprobe timeout; targeted liên quan **18/18 pass**, full CI **117/117 pass**.
- **Bước 17 PASS:** branch `codex/step17-revalidation`, PR [#18](https://github.com/nkdang2772/video_gensys/pull/18), 2/2 GitHub checks xanh và merge commit `b756804` trên `main`.
- Bằng chứng Bước 17: chỉnh inline `visual_description` 10 shot qua cùng persistence path của UI; filter Scene 1 đúng 10 shot; bulk assign character + primary cho 20 shot; chosen audio preview được render. Không chọn shot khi bulk hiển thị lỗi (BUG-040). Targeted **13/13 pass**, full CI **120/120 pass**.
- **Bước 18 PASS:** branch `codex/step18-revalidation`, PR [#19](https://github.com/nkdang2772/video_gensys/pull/19), 2/2 GitHub checks xanh và merge commit `d779578` trên `main`.
- Bằng chứng Bước 18: AppTest tạo 6 character reference series-specific qua UI, thêm version v1 cho từng reference, tạo Episode và xác nhận đủ 6 `EpisodeReferencePin`; thiếu source version hiển thị lỗi và không tạo `ReferenceVersion`. Targeted **15/15 pass**, full CI **122/122 pass**.
- **Bước 19 PASS:** branch `codex/step19-revalidation`, PR [#20](https://github.com/nkdang2772/video_gensys/pull/20), 2/2 GitHub checks xanh và merge commit `0c45792` trên `main`.
...
- **Bước 23 PASS:** branch `codex/step23-revalidation`, PR [#25](https://github.com/nkdang2772/video_gensys/pull/25), 2/2 GitHub checks xanh và merge commit `0ae231d` trên `main`.
